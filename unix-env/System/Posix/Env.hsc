...
getEnv name = do
  litstring <- S.useAsOSString name c_getenv
  if litstring /= nullPtr
     then liftM Just $ S.packOSString litstring
     else return Nothing
...
getEnvironment = do
  ...
  return $ map (dropEq . (S.break ((==) '='))) env
 where
   dropEq (x,y)
      | S.head y == '=' = (x,S.tail y)
      | otherwise       = error $ "getEnvironment: insane variable " ++ S.unpack x
...
unsetEnv name
    ...
    = S.useAsOSString name $ \ s ->
        throwErrnoIfMinus1_ "unsetenv" (c_unsetenv s)
    | haveUnsetEnv && unsetEnvReturnsVoid
    = S.useAsOSString name c_unsetenv_void
    | otherwise
    = putEnv (S.append name (S.pack "="))

-- |'putEnv' function takes an argument of the form @name=value@
-- and is equivalent to @setEnv(key,value,True{-overwrite-})@.

putEnv :: Str -> IO ()
putEnv keyvalue = do s <- S.newOSString keyvalue
...
setEnv key value ovrwrt
 | haveSetEnv = do
  S.useAsOSString key $ \ keyP ->
    S.useAsOSString value $ \ valueP ->
      throwErrnoIfMinus1_ "setenv" $
        c_setenv keyP valueP (fromIntegral (fromEnum ovrwrt))
 | otherwise =
  case ovrwrt of
    True -> putEnv (key `S.append` S.pack "=" `S.append` value)
    False -> do
      res <- getEnv key
      case res of
        Just _  -> return ()
        Nothing -> putEnv (key `S.append` S.pack "=" `S.append` value)
